...
HP_FLOAT = "float"
HP_POS_FLOAT = "pos_float"         # > 0, no acotado a 1
HP_NONNEG_FLOAT = "nonneg_float"   # >= 0, no acotado a 1
...
def optuna_suggest_pos_float(trial: Trial, name: str) -> float:
    # Positivo, rango amplio. Log=True para cubrir órdenes de magnitud.
    return trial.suggest_float(name, 1e-6, 1e2, log=True)

def optuna_suggest_nonneg_float(trial: Trial, name: str) -> float:
    # No negativo, rango amplio. (No log porque incluye 0)
    return trial.suggest_float(name, 0.0, 1e1)
...
HP_OPTUNA: Dict[str, TrialSuggestionFunctionType] = {
    ...
    HP_POS_FLOAT: optuna_suggest_pos_float,
    HP_NONNEG_FLOAT: optuna_suggest_nonneg_float,
}
